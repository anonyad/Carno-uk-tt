...
var routes = [
  ...
  {
    path: "board",
    loadComponent: () => import("./chunk-AWYD4U6R.js").then((m) => m.TaskBoard)
  }
];
...
var StoreDevtools = class _StoreDevtools {
  constructor(dispatcher, actions$, reducers$, extension, scannedActions, errorHandler2, initialState, config) {
    const liftedInitialState = liftInitialState(initialState, config.monitor);
    const liftReducer = liftReducerWith(initialState, liftedInitialState, errorHandler2, config.monitor, config);
...
var TaskService = class _TaskService {
   ...
   * TODO: Replace with actual API calls using HttpClient
   */
  mockTasks = [
    {
      id: "1",
   ...
   */
  addTask(taskDto) {
    const newTask = __spreadProps(__spreadValues({
  ...
  updateTask(taskDto) {
    ...
    const updatedTask = __spreadProps(__spreadValues(__spreadValues({}, this.mockTasks[index]), taskDto), {
      updatedAt: /* @__PURE__ */ new Date()
    });
   ...
   *
   * Simulates a 10% failure rate to demonstrate rollback strategy
   */
  moveTask(move) {
    const shouldFail = Math.random() < 0.1;
    if (shouldFail) {
      return throwError(() => new Error("Network error: Failed to move task")).pipe(delay(300));
    }
    ...
    this.mockTasks = this.mockTasks.map((t) => t.id === move.taskId ? updatedTask : t);
    return of(updatedTask).pipe(delay(300));
  }
  /**
   * Delete a task
...
var TasksEffects = class {
   ...
   * Load Tasks Effect
   *
   * Pattern: Pessimistic update (wait for server response)
   * Error handling: Catch and dispatch failure action
   */
  loadTasks = createEffect(() => this.actions.pipe(ofType(TasksActions.load_Tasks), switchMap(({ boardId }) => this.taskService.loadTasks(boardId).pipe(map((tasks) => TasksActions.load_Tasks_Success({ tasks })), catchError((error) => of(TasksActions.load_Tasks_Failure({ error: error.message })))))));
  /**
   * Add Task Effect
   *
   * TODO: Implement add task effect
   * Pattern: Pessimistic (wait for server to create task)
   */
  // addTask$ = createEffect(() => ...);
  /**
   * Update Task Effect
   *
   * TODO: Implement update task effect
   * Pattern: Pessimistic (wait for server confirmation)
   */
  // updateTask$ = createEffect(() => ...);
  /**
   * Move Task Effect - OPTIMISTIC UPDATE
   *
   * This is the key effect that demonstrates optimistic updates with rollback
   *
   * Flow:
   * 1. User triggers Move Task action
   * 2. Immediately dispatch Move Task Optimistic (updates UI instantly)
   * 3. Call API in background
   * 4. On success: Dispatch Move Task Success (confirm the change)
   * 5. On failure: Dispatch Move Task Failure (triggers rollback in reducer)
   */
  moveTask = createEffect(() => this.actions.pipe(
    ofType(TasksActions.move_Task),
    // Use concatMap to ensure order: optimistic update → API call
    concatMap(({ move }) => {
      this.store.dispatch(TasksActions.move_Task_Optimistic({ move }));
      return this.taskService.moveTask(move).pipe(
        // 3. On success: Dispatch success action (confirms the change)
        map((task) => TasksActions.move_Task_Success({ task })),
        // 4. On failure: Dispatch failure action (triggers rollback in reducer)
        catchError((error) => of(TasksActions.move_Task_Failure({
          error: error.message,
          move
          // Pass original move data for rollback
        })))
      );
    })
  ));
...
// src/app/app.ts
var App = class _App {
  title = signal("Carno-Uk", ...ngDevMode ? [{ debugName: "title" }] : []);
